...
The bundled client runs offline-mode: `launch-client.sh` sets `hmc.offline=true` and passes `-offline` to HeadlessMC's `launch` command. The user has no premium Minecraft account, and none is needed — offline-mode clients can still join any server with `online-mode=false`, which is what this is for.

The username is configurable rather than fixed. `McClient` owns the current value (`MCCLI_USERNAME`, default `LLMBotClient`) and passes it, plus a derived UUID, to `launch-client.sh` as `MCCLI_USERNAME`/`MCCLI_UUID` env vars when it spawns the process; the script forwards them as `hmc.offline.username`/`hmc.offline.uuid` (confirmed against `LauncherProperties.java` in HeadlessMC's own source). The UUID is not random: it is `UUID.nameUUIDFromBytes` over `"OfflinePlayer:" + username` — MD5, version 3, RFC 4122 variant — the same derivation vanilla Minecraft servers use for offline players, so a given username always maps to the same identity a server would compute. `client-set-username` changes it at runtime; since the username is a JVM property fixed at launch, changing it restarts the client process if one is running (the tool's response says whether it restarted), or just records the value for the next launch if not.
...
| `MCCLI_USERNAME` | `LLMBotClient` | Real client's in-game username; also settable at runtime with `client-set-username` |
